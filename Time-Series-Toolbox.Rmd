---
title: "Homework 1 - Time Series Toolbox"
author: "Prof. Dr. Michael Wenk"
date: 'Submission date: 17.12.2020'
output:
  html_document:
    theme: cerulean
    css: styles/homework.css
    toc: yes
    toc_depth: 3
    toc_float: yes
  word_document:
    toc: yes
    toc_depth: '3'
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, cache = TRUE)
options(digits = 5)

# libraries to load
library(fpp3)
library(fpp2)
```

# Exercise 1

For the following series, find an appropriate Box-Cox (?BoxCox) transformation in order to stabilize the variance. First plot the time series and decide which of them need a transformation. 

  * `usnetelec`
  * `usgdp`
  * `mcopper`
  * `enplanements`

```{r}
# todo
autoplot(usnetelec)
autoplot(usgdp)
autoplot(mcopper)
autoplot(enplanements)
```


```{r}
#usnetelec und usgdp benötigen keine Transformation. mcopper und enplanements sind sehr volatil und haben eine Transformation nötig.

#Für mcopper liefert lambda=0.5 das übersichtlichste Ergebnis.
#Für enplanements liefert lambda=0 das übersichtlichste Ergebnis.
```


```{r}
autoplot(BoxCox(mcopper, lambda=-1)) 
autoplot(BoxCox(mcopper, lambda=-0.5)) 
autoplot(BoxCox(mcopper, lambda=0)) 
autoplot(BoxCox(mcopper, lambda=0.5)) 
autoplot(BoxCox(mcopper, lambda=1)) 

```
```{r}
autoplot(BoxCox(enplanements, lambda=-1)) 
autoplot(BoxCox(enplanements, lambda=-0.5)) 
autoplot(BoxCox(enplanements, lambda=0)) 
autoplot(BoxCox(enplanements, lambda=0.5)) 
autoplot(BoxCox(enplanements, lambda=1)) 
```


# Exercise 2

Why is a Box-Cox transformation unhelpful for the `cangas` data?
```{r}

```

```{r}
# todo
help(cangas) 
autoplot(cangas)
autoplot(BoxCox(cangas, lambda=-1)) 
autoplot(BoxCox(cangas, lambda=-0.5)) 
autoplot(BoxCox(cangas, lambda=0)) 
autoplot(BoxCox(cangas, lambda=0.5)) 
autoplot(BoxCox(cangas, lambda=1))
```
Your answer: Die Box-Cox Transformationen bringen cangas, bzw. die seasonal variation, in eine weniger übersichtliche Form. Daher ist es nicht hilfreich.



# Exercise 3

What Box-Cox transformation would you select for **your** retail data from Exercise 4 in Visualization?

```{r}
# Example 
retaildata <- readxl::read_excel("data/retail.xlsx", skip=1)
myts <- ts(retaildata[, "A3349873A"], frequency = 12, start = c(2000, 5))#myts <- ts(retaildata[,"A3349873A"], frequency=12, start=c(1982,4))
autoplot(myts)
#myts %>% BoxCox(lambda = 0) %>% autoplot()

autoplot(BoxCox(myts, lambda=-1)) 
autoplot(BoxCox(myts, lambda=-0.5)) 
autoplot(BoxCox(myts, lambda=0)) 
autoplot(BoxCox(myts, lambda=0.1)) 
autoplot(BoxCox(myts, lambda=0.5)) 
autoplot(BoxCox(myts, lambda=1))
```

From visual inspection, a log transformation would be appropriate here. It also makes sense, as retail expenditure is likely to increase proportionally to population, and therefore the seasonal fluctuations are likely to be proportional to the level of the series. It has the added advantage of being easier to explain than some other transformations. Finally, it is relatively close to the automatically selected value of `BoxCox.lambda(myts)` $= `r round(BoxCox.lambda(myts),3)`$.

If you have selected a different series from the retail data set, you might choose a different transformation.

Your answer: Lambda=0.1 liefert das übersichtlichste Ergebnis.


# Exercise 4

Calculate the residuals (?residuals) from a seasonal naive forecast applied to the quarterly Australian beer production data from 1992. The following code will help.

```{r}
# todo
?residuals
beer <- as_tsibble(window(ausbeer, start=1992))
autoplot(beer)
beer_fit <- beer %>% model(naive = NAIVE(value), mean = MEAN(value))
augment(beer_fit)

```


```{r}

```

Test if the residuals are white noise and normally distributed.

```{r}
#checkresiduals(fc)
#mo21  <-  tsglm(x,model=list(past_obs=c(1,2)),distr="poisson")
checkresiduals(auto.arima(beer), plot = FALSE)
#augment(beer) %>% checkresiduals(.resid)
```


```{r}
augment(beer_fit) %>%
  autoplot(.resid) + xlab("Year") + ylab("") +
  ggtitle("Residuals from snaïve Model")

augment(beer_fit) %>%
ACF(.resid) %>%
autoplot() + ggtitle("ACF of residuals")

```

What do you conclude?

Your answer: Die Werte schwanken stark. Sie gehen von 120 bis -120 im Mittelwert und von 100 bis -60 bei naive.Es ist kein Rauschen.


# Exercise 5

Are the following statements true or false? Explain your answer.

> a. Good forecast methods should have normally distributed residuals.

Your answer: 
True. 
Residuals in forecasting: difference between observed value and its fitted value.
Most time series models produce normally distributed forecasts.
The forecast distribution describes the probability of observing any future value.

> b. A model with small residuals will give good forecasts.

Your answer:
True.
Residuals in forecasting: difference between observed value and its fitted value.
-> Geringe Unterschiede sind besser als Große.

> c. The best measure of forecast accuracy is MAPE.

Your answer:
False.
MAPE is scale independent but is only sensible if ???????? ??? 0 for all ????,
and ???? has a natural zero.
-> Nicht immer das Beste.

> d. If your model doesn't forecast well, you should make it more complicated.

Your answer:
False.
Die Vorverarbeitung der Daten ist häufig wichtiger als das Modell.

> e. Always choose the model with the best forecast accuracy as measured on the test set.

Your answer:
False.
Die forecast Genauigkeit deckt nicht alle Fälle perfekt ab.
Extremwerte können es beispielsweise beeinflussen.

# Exercise 6

For your retail time series (from Exercise 4):

Split the data into two parts using

```{r}
retaildata <- readxl::read_excel("data/retail.xlsx", skip=1)
#myts <- ts(retaildata[,"A3349873A"], frequency=12, start=c(1982,4))
myts.train <- as_tsibble(window(myts, end=c(2010,12)))
myts.test <- as_tsibble(window(myts, start=2011))
```

Check that your data have been split appropriately by producing the following plot.

```{r}
myts <- ts(retaildata[,"A3349873A"], frequency=12, start=c(1982,4))
myts.train <- as_tsibble(window(myts, end=c(2010,12)))
myts.test <- as_tsibble(window(myts, start=2011))

autoplot(myts)
autoplot(myts.train)
autoplot(myts.test)

#autoplot(myts) +
#  autolayer(myts.train, series="Training") +
#  autolayer(myts.test, series="Test")
```

Calculate forecasts using `snaive` applied to `myts.train`.

```{r}
# todo
erg <- myts.train %>%
  model(
    Seasonal_naive = SNAIVE(value)
  )

```

Compare the accuracy of your forecasts against the actual values stored in `myts.test`.
(?accuracy) 

```{r}
# todo
erg_fc <- erg %>%
  forecast()
accuracy(erg) 

erg2 <- myts.test %>%
  model(
    Seasonal_naive = SNAIVE(value)
  )
accuracy(erg2)
 
```

The number to look at here is the test set RMSE of 71.443. That provides a benchmark for comparison when we try other models.

Check the residuals. Do the residuals appear to be uncorrelated and normally distributed?

```{r}
# todo
autoplot(myts.test)
autoplot(erg_fc)

augment(erg) %>%
  autoplot(.resid) + xlab("Year") + ylab("") +
  ggtitle("Residuals from snaïve Model")



```

Your answer: Die Werte sind weitgehend normalverteilt. Es ist von einer Korrelation auszugehen.


How sensitive are the accuracy measures to the training/test split? Je weniger Testdaten vorhanden sind, desto sensitiver wird es.


# Exercise 7

`visnights` contains quarterly visitor nights (in millions) from 1998-2015 for eight regions of Australia.

Use `window()` to create three training sets for `visnights[,"QLDMetro"],` omitting the last 1, 2 and 3 years; call these train1, train2, and train3, respectively.

```{r}
train1 <- as_tsibble(window(visnights[, "QLDMetro"], end = c(2015, 3)))
train2 <- as_tsibble(window(visnights[, "QLDMetro"], end = c(2015, 2)))
train3 <- as_tsibble(window(visnights[, "QLDMetro"], end = c(2015, 1)))
```

Compute one year of forecasts for each training set using the `snaive()` method. Call these `fc1`, `fc2` and `fc3`, respectively.

```{r}
# todo

fc1 <- train1 %>%
  model(
    Seasonal_naive = SNAIVE(value)
  )

fc2 <- train2 %>%
  model(
    Seasonal_naive = SNAIVE(value)
  )

fc3 <- train3 %>%
  model(
    Seasonal_naive = SNAIVE(value)
  )

f1 <- fc1 %>% forecast(h=1)
f2 <- fc2 %>% forecast(h=1)
f3 <- fc3 %>% forecast(h=1)
```

Use `accuracy()` to compare the MAPE over the three test sets. Comment on these.
First we will copy the actual data into a variable. Then we can do an accuracy comparison.

```{r}
qld <- visnights[, "QLDMetro"]
# todo
#accuracy(qld)
accuracy(fc1) #MAPE = 8.0937
accuracy(fc2) #MAPE = 8.1201
accuracy(fc3) #MAPE = 8.1712

# Für train1 ist MAPE am höchsten und für train 3 am niedrigsten.
```


```{r}

```

This should give similar results to this consolidated results table.

```
                     ME     RMSE      MAE        MPE     MAPE      MASE       ACF1
Training set  0.1637836 1.742687 1.360271  0.4384347 7.357322 1.0000000 0.06643175
Test set fc1 -1.3010774 1.301077 1.301077 -6.9956861 6.995686 0.9564839         NA
Test set fc2 0.08383478 1.387447 1.384912 -0.4063445 6.589342 1.019346 -0.50000000
Test set fc3 0.06202858 1.132896 0.9294135 -0.237857 4.425934 0.6738562 -0.51548610
```

The lower MAPE value for "fc3" indicates a better result when we use the previous 3 values for the `snaive()` prediction.

# Exercise 8

Use the Dow Jones index (data set `dowjones`) to do the following:

Produce a time plot of the series.

```{r}
# todo
autoplot(dowjones) 
```

Produce forecasts using the drift method and plot them.

Let's assume we want to forecast the next 5, 10 and 15 values.

```{r}
dowfc1 <- rwf(dowjones, drift=TRUE, h=5)
# todo
autoplot(dowfc1) 
dowfc2 <- rwf(dowjones, drift=TRUE, h=10)
autoplot(dowfc2) 
dowfc3 <- rwf(dowjones, drift=TRUE, h=15)
autoplot(dowfc3) 
```

Then we can plot these values.

```{r eval = FALSE}
# remove eval=FALSE
autoplot(dowjones) +
  autolayer(dowfc1, PI=FALSE, series="Drift 5") +
  autolayer(dowfc2, PI=FALSE, series="Drift 10") +
  autolayer(dowfc3, PI=FALSE, series="Drift 15") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
```

We show that the forecasts are identical to extending the line drawn between the first and last observations.

We can plot the forecasts in a different order, so the shorter forecasts are superimposed, showing the lines are the same.

```{r eval = FALSE}
# remove eval=FALSE
autoplot(dowjones) +
  autolayer(dowfc3, PI=FALSE, series="Drift 15") +
  autolayer(dowfc2, PI=FALSE, series="Drift 10") +
  autolayer(dowfc1, PI=FALSE, series="Drift 5") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
```

Try using some of the other benchmark functions to forecast the same data set. Which do you think is best? Why? Drift schneidet von der Accuracy am besten ab. Der Abstand zu den anderen Methoden ist allerdings sehr gering.

The time series isn't seasonal, so the seasonal naive method is not viable. However, we can use the mean and naive methods.

```{r}
# todo
dowfc1NAIVE <- rwf(dowjones, NAIVE=TRUE, h=5)
autoplot(dowfc1NAIVE) 
dowfc2NAIVE <- rwf(dowjones, NAIVE=TRUE, h=10)
autoplot(dowfc2NAIVE) 
dowfc3NAIVE <- rwf(dowjones, NAIVE=TRUE, h=15)
autoplot(dowfc3NAIVE)  

dowfc1MEAN <- rwf(dowjones, MEAN=TRUE, h=5)
autoplot(dowfc1MEAN) 
dowfc2MEAN <- rwf(dowjones, MEAN=TRUE, h=10)
autoplot(dowfc2MEAN) 
dowfc3MEAN <- rwf(dowjones, MEAN=TRUE, h=15)
autoplot(dowfc3MEAN)  
```


```{r}
accuracy(dowfc1)
accuracy(dowfc1NAIVE)
accuracy(dowfc1MEAN)
```

The three values will be very different here. The Mean will use the data set, so is unlikely to follow the current trendline.

# Exercise 9

Consider the daily closing IBM stock prices (data set `ibmclose`).

Produce some plots of the data in order to become familiar with it.

```{r}
# todo
autoplot(ibmclose) 
```

Split the data into a training set of 300 observations and a test set of 69 observations.

```{r}
# todo

ibmclose.train <- as_tsibble(window(ibmclose, end=c(300)))
ibmclose.test <- as_tsibble(window(ibmclose, start=c(301))) 

ibm.train <- window(ibmclose, end=c(300))
ibm.test <- window(ibmclose, start=c(301)) 


ibm1 <-ts(ibmclose)
ibm1.train <-window(ibmclose, end=c(300))
ibm1.test <- window(ibmclose, start=c(301))

```

Try using various benchmark methods to forecast the training set and compare the results on the test set. Which method did best?

```{r eval = FALSE}
h <- length(ibm1.test)
m.f <- meanf(ibm1.train, h=h)
# todo
fc <- ibmclose.train %>%
  model(
    #Seasonal_naive = SNAIVE(value),
    Naive = NAIVE(value),
    Drift= RW(value ~ drift()),
    Mean = MEAN(value)
  ) 
rwd.f = rwf(ibm1.train, drift=TRUE, h=h)
rw.f = rwf(ibm1.train, naive=TRUE, h=h)

autoplot(ibmclose) +
  xlab("Day") +
  ggtitle("Daily closing IBM stock prices") +
  autolayer(m.f$mean, col=2, series="Mean method") +
  autolayer(rw.f$mean, col=3, series="Naive method") +
  autolayer(rwd.f$mean, col=4, series="Drift method")

# accuracy(m.f,ibm.test)
# todo
accuracy(fc)
accuracy(m.f)
accuracy(rw.f)
accuracy(rwd.f)

```


Check the residuals of your preferred method. Do they resemble white noise? Die bevorzugte Methode Drift hat Rauchen. Die 2 Durchbrüche sind statistisch nicht signifikant.

```{r}
# todo
augment(fc) %>%
  autoplot(.resid) + xlab("Year") + ylab("") +
  ggtitle("Residuals from snaïve Model")


augment(fc) %>%
ACF(.resid) %>%
autoplot() + ggtitle("ACF of residuals")

```


# Exercise 10

Consider the sales of new one-family houses in the USA, Jan 1973 -- Nov 1995 (data set `hsales`).

Produce some plots of the data in order to become familiar with it.

```{r}
# todo
autoplot(hsales) 
```

Split the `hsales` data set into a training set and a test set, where the test set is the last two years of data.

```{r}
# todo
#hsales1 <- ts(hsales)
hsales.train <- as_tsibble(window(hsales, end=c(1992)))
hsales.test <- as_tsibble(window(hsales, start=c(1993))) 

hsales1.train <- window(hsales, end=c(1992))
hsales1.test <- window(hsales, start=c(1993))

#hsales11 <- ts(hsales)
#hsales11.train <- as_tsibble(window(hsales11, end=c(1993,12)))
#hsales11.test <- as_tsibble(window(hsales11, start=c(1994,12))) 
```

Try using various benchmark methods to forecast the training set and compare the results on the test set. Which method did best? Drift schneidet am besten ab.

```{r}
# todo
h <- length(hsales1.test)
m.f <- meanf(hsales1.train, h=h)
# todo
fc <- hsales.train %>%
  
  model(
    #Seasonal_naive = SNAIVE(value),
    Naive = NAIVE(value),
    Drift= RW(value ~ drift()),
    Mean = MEAN(value)
  ) 

erg_fc <- fc %>%
  forecast()
accuracy(fc)
autoplot(erg_fc)



```


```{r}

h=length(hsales1.test)

meanfc <- meanf(hsales1.train, h=h)
driftfc = rwf(hsales1.train, drift=TRUE, h=h)
naivefc = rwf(hsales1.train, naive=TRUE, h=h)

autoplot(meanfc)
autoplot(driftfc)

autoplot(naivefc)
autoplot(hsales1.train)


```

In terms of accuracy measures on the test set, the seasonal naive method does better.

Check the residuals of your preferred method. Do they resemble white noise? Bei Drift handelt es sich um Rauschen. Der eine deutliche Durchbruch ist nicht signifikant.

```{r eval = FALSE}
# remove eval=FALSE
checkresiduals(m.f)



augment(fc) %>%
  autoplot(.resid) + xlab("Year") + ylab("") +
  ggtitle("Residuals from snaïve Model")


augment(fc) %>%
ACF(.resid) %>%
autoplot() + ggtitle("ACF of residuals")


```


